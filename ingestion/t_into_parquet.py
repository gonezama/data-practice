...
def partitioning(datetime_col, output_path):
    return PartitionByKey(
        output_path,
        by=[
            pl.col(datetime_col).dt.year(),
            pl.col(datetime_col).dt.month(),
            pl.col(datetime_col).dt.day(),
            pl.col(datetime_col).dt.hour(),
        ],
...
def lazy_csv_to_parquet(
    ...
    ).unique().sink_parquet(partitioning(datetime_col, output_path))
